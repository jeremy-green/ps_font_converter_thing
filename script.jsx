    ...
    // currentLayer is not a LayerSet
      ...
      if ( (currentLayer.visible) && (currentLayer.kind == LayerKind.TEXT) ) {
        ...
        for (prop in textObj) {
          ...
          if (val) {
            definitionsArr.push(formatSelector({
...
function formatVariable(name) {
  return '$' + getMachineName(name);
}

function formatVariableDefinition(name, value) {
  return formatVariable(name) + ': ' + value + ';';
}
...
function getFont(font) {
  ...
  return {
    ...
    'italic': (style[1] != undefined) ? 'italic' : false
  };
}
